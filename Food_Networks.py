...
# This block of code will take a few hours to run so you may want to get coffee haha :)
...
############################### A_Z_RECIPE COLLECTION ######################################
# a-z recipes from each alphabet page so iterate over 26 pages 

def a_z_recipes():
    client = pymongo.MongoClient("mongodb://localhost:27017/")
    all_fn = client["FoodNetworkData"]
    all_recipes = all_fn["a_z_recipes"]
    
    file_names_az = os.listdir(path_fn_az)
    # Create unique index on recipe ID
    all_recipes.create_index("recipe_id", unique=True)
    
    for i, num in enumerate(file_names_az):
        with(open(path_fn_az + num, 'r', encoding = 'utf-8')) as file_html:
            soup_fn = bs(file_html.read(), "html.parser")      
           
            recipe_links = soup_fn.find('div', {'class': 'aToZ section'}).findAll("section", attrs = {'class' : 'o-Capsule o-SiteIndex'})
            for a, link in enumerate(recipe_links):
                ...
                for item, ch_link in enumerate(link.findAll('li', {'class': 'm-PromoList__a-ListItem'})):
                    recipes_dict = {}
                    recipes_dict["recipe_id"] = "REC_" + alphabet + "_" + str(item)
                    recipes_dict["recipe_name"] = ch_link.text.strip()
                    recipes_dict["recipe_url"] = "https:" + ch_link.find("a").get("href")
                    print(recipes_dict)
                    all_recipes.insert_one(recipes_dict)
